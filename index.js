...
fs.copyFile(
  ...
  path.resolve(hexo.public_dir, "js/lazyload/lazyload.js")
);
// copy default loading.gif to pnlic dir
if (!hexo.config.lazyload.loadingImg) {
  fs.copyFile(
    path.resolve(__dirname, "loading.gif"),
    path.resolve(hexo.public_dir, "images/loading.gif")
  );
}
...
if (hexo.config.lazyload.onlypost) {
  ...
  hexo.extend.filter.register(
    "after_render:html",
    require("./lib/process").processSite
  );
}
hexo.extend.filter.register(
  "after_render:html",
  require("./lib/addscripts").addScript
);
